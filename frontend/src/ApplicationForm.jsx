...
const ApplicationForm = ({ job, onClose, onSubmit }) => {
  ...
  const handleAddSlot = () => {
    if (slots.length >= 10) {
      setFormError('You can add up to 10 interview availability slots.');
      return;
    }
    setFormError('');
  ...
  const handleRemoveSlot = () => {
    if (slots.length <= 3) {
      setFormError('Please keep at least 3 interview availability slots.');
      return;
    }
    setFormError('');
  ...
  const handleSubmit = async (event) => {
    ...
    if (slots.length < 3) {
      setFormError('Please add at least 3 interview availability slots before submitting.');
      return;
    }
    ...
    for (let i = 0; i < slots.length; i++) {
      ...
      if (!slot.date || !slot.startTime || !slot.endTime) {
        setFormError(`Slot ${i + 1} is incomplete. Please choose a date and both start/end times.`);
        return;
      }
          ...
          <section style={sectionStyle}>
            ...
            <div style={{ marginTop: '16px', padding: '12px 14px', backgroundColor: '#dbeafe', borderLeft: '4px solid #0284c7', borderRadius: '6px' }}>
              <p style={{ margin: 0, color: '#0c4a6e', fontSize: '0.9rem' }}>
                <strong>Required:</strong> Please select at least 3 interview time slots, and you can add up to 10.
              </p>
            </div>
            <div style={{ marginTop: '18px', display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
              <button type="button" onClick={handleAddSlot} disabled={slots.length >= 10} style={{ padding: '8px 12px', border: '1px solid #667eea', borderRadius: '8px', backgroundColor: '#fff', color: '#667eea', cursor: slots.length >= 10 ? 'not-allowed' : 'pointer', fontWeight: '700' }}>
                + Add slot
              </button>
              <button type="button" onClick={handleRemoveSlot} disabled={slots.length <= 3} style={{ padding: '8px 12px', border: '1px solid #d1d5db', borderRadius: '8px', backgroundColor: '#fff', color: slots.length <= 3 ? '#9ca3af' : '#374151', cursor: slots.length <= 3 ? 'not-allowed' : 'pointer', fontWeight: '700' }}>
                − Remove last slot
              </button>
            </div>
